...
//               <h1 className="text-white text-2xl font-bold">DSH</h1>
//             </Link>
//           </div>
...
//         </nav>
//         {mobileMenuOpen && (
//           <div className="fixed inset-0 z-50 flex flex-col bg-black bg-opacity-70 backdrop-blur-md px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
...
export default function Navbar() {
  ...
  }, [location]); // Watch for route changes
                ...
                <div className="space-y-2 py-6">
                  {navigation.map((item) => (
                    <Link
                      ...
                      to={item.href}
                      className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-white hover:bg-gray-700"
                      onClick={() => setMobileMenuOpen(false)} // Optionally auto-close on click
                    >
                      {item.name}
                    </Link>
                  ))}
